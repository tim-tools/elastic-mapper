package io.elasticmapper.example.mapper;
...
import io.elasticmapper.example.dto.StatusCounts;
import io.elasticmapper.example.entity.User;
...
public interface UserMapper extends BaseMapper<User> {
    ...
    io.elasticmapper.executor.ESResponse updateStatus(@Param("status") String status);

    /**
     * Delete users by status (annotation-driven).
     */
    @Delete("{\"query\":{\"term\":{\"status\":\"#{status}\"}}}")
    io.elasticmapper.executor.ESResponse deleteByStatus(@Param("status") String status);
